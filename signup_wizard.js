// ملاحظة هامة: احتفظ بنفس اسم الملف الذي تستخدمه حالياً في وسم <script> بصفحة
// التسجيل عند استبدال المحتوى، حتى لا ينكسر الرابط بين الصفحة والسكربت.

var CONFIG_APP = window.CONFIG_APP;

// دالة موحّدة وآمنة للحصول على عميل Supabase. تُستدعى عند كل استخدام بدل قراءة
// window.supabaseClient مرة واحدة عند تحميل السكربت (كما كان سابقاً) — تلك القراءة
// المبكرة كانت تفشل بصمت إن لم تكن مكتبة Supabase ومتغيّر العميل جاهزين في تلك
// اللحظة بالضبط، مما يجعل كل عمليات التسجيل/الدخول تتوقف دون رسالة خطأ واضحة.
function getClient() {
...
function initAuthStep() {
    ...
    googleBtn?.addEventListener('click', async () => {
        ...
        if (!client) {
            setAuthMessage('تعذّر الاتصال بقاعدة البيانات، يرجى تحديث الصفحة والمحاولة مجدداً.', 'error');
            return;
        }
...
function saveDraft() {
    ...
    try { localStorage.setItem(CONFIG_APP.DRAFT_KEY, JSON.stringify(draft)); } catch (e) { console.warn('تعذّر حفظ المسودة محلياً:', e); }
}


function restoreDraft() {
    let raw;
    try { raw = localStorage.getItem(CONFIG_APP.DRAFT_KEY); } catch (e) { console.warn('تعذّر قراءة المسودة المحفوظة:', e); return; }
    if (!raw) return;
    let draft;
    try { draft = JSON.parse(raw); } catch (e) { console.warn('تعذّر تحليل المسودة المحفوظة:', e); return; }
...
async function uploadFileToStorage(folder, file) {
    const client = getClient();
    if (!client) throw new Error('تعذّر الاتصال بقاعدة البيانات.');
    ...
    const { error: uploadError } = await client.storage.from('media').upload(filePath, file, {
        cacheControl: '3600', upsert: false
    });
    if (uploadError) throw new Error(`فشل رفع الملف (${file.name}): ${uploadError.message}`);


    const { data: urlData } = client.storage.from('media').getPublicUrl(filePath);
...
async function cleanupUploadedFiles(filePaths) {
    ...
    const client = getClient();
    if (!client) return;
    try { await client.storage.from('media').remove(filePaths); } catch (e) { console.warn('تعذّر إلغاء الملفات المرفوعة:', e); }
}
...
function initFinalSubmit() {
    document.getElementById('signupForm')?.addEventListener('submit', async function (e) {
        ...
        const client = getClient();
        if (!client) {
            alert('تعذّر الاتصال بقاعدة البيانات. يرجى تحديث الصفحة والمحاولة مجدداً.');
            return;
        }
        ...
        try {
            ...
            const { error: dbError } = await client.from('profiles').upsert([payload]);
...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    const client = getClient();
    if (!client) {
        setAuthMessage('تعذّر الاتصال بقاعدة البيانات. تأكد من تحميل مكتبة Supabase JS في هذه الصفحة قبل هذا السكربت، أو أعد تحميل الصفحة.', 'error');
        return;
    }
